...
<cfdi:Comprobante
        ...
        <#elseif ComercioE == true>
            ...
            <#assign xsischemalocation_CE = " http://www.sat.gob.mx/ComercioExterior20 http://www.sat.gob.mx/sitio_internet/cfd/ComercioExterior20/ComercioExterior20.xsd">
        ...
        <#if transaction.shippingcost?length gt 0>
            ...
            <#if desglosa_ieps == true>
                SubTotal="${desglose_cab.subtotal_gbl}"
            ...
            Total="${desglose_cab.total_netsuite}"
        ...
        <#else>
            ...
            <#--  Total="${desglose_cab.total}"  -->
            Total="${desglose_cab.total_netsuite}"
            <#assign TotalCab = (((desglose_cab.subtotal?number)-total_desc_cabecera)+desglose_cab.totalImpuestos?number)>
            <#--  Total="${TotalCab?string["0.00"]}"  -->
        </#if>
...
Descuento="${total_desc_cabecera}">
 ...
 Descuento="${desglose_cab.descuento}">
                ...
                <#if item.itemtype != "Group" && item.itemtype != "EndGroup" && item.itemtype != "Discount" && item.itemtype != "Description" && customItem.quantity?number gt 0>
                    <cfdi:Concepto
                        ...
                        <#if item.custcol_efx_invdet_json?has_content>
                            ...
                            <#if transaction.custbody_fb_add_in_desc>
                        ...
                        <#if desglosa_ieps == true>
                        <#if ((customItem.amount?number) != (desglose.montoLinea?number))>
                                Importe="${desglose.montoLinea?number?string["0.00"]}"
                            <#else>
                        ...
                        <#if json_descuentos?number gt 0>
                            <#if desglosa_ieps == false && desglose.iva.name?has_content && desglose.ieps.name?has_content && desglose.iva.rate_div?number==0>
                                Descuento="${desglose.iva.descuento}"
                                ...
                                <#if desglose.iva.name?has_content || desglose.ieps.name?has_content || desglose.exento.name?has_content>
                                    <cfdi:Traslados>
                                        ...
                                        <#if desglose.iva.name?has_content>
                                            ...
                                            <#assign tiene_impuesto = tiene_impuesto + 1>
                                            <cfdi:Traslado Base="${desglose.iva.base_importe?number?string["0.00"]}" Importe="${desglose.iva.importe}" Impuesto="002" TasaOCuota="${desglose.iva.rate_div?number?string["0.000000"]}" TipoFactor="Tasa" />
                                        </#if>
...
<#assign tieneobjimp = tieneobjimp+1>
                ...
                <cfdi:Concepto
                    ...
                    <#assign "impuestoLineashipp" = impuestoLineashipp+impuesto_envio_total>
                        <#if impuesto_envio_total gt 0>
                                ...
                                <cfdi:Impuestos>
                                    <cfdi:Traslados>
                                        <cfdi:Traslado Base="${transaction.shippingcost?string["0.00"]}" Importe="${desglose_cab.shipping_tax}" Impuesto="002" TasaOCuota="${tasa_envio?number?string["0.000000"]}" TipoFactor="Tasa" />
    ...
    <#--  <#if tieneobjimp gt 0>  -->
    <#--  <#if desglose_cab.shipping_cost !="0">  -->
                <cfdi:Impuestos TotalImpuestosTrasladados="${desglose_cab.totalTraslados}">
    <#--  </#if>  -->
        <#if desglosa_ieps == true>
            ...
            <#elseif desglose_cab.ieps_total != "0.00" || desglose_cab.shipping_cost != "0.00" || total_traslados_ivacero gt 0 || tieneExento == true>
                <#assign exentoContador = exentoContador+1>
        ...
        <#else>
            ...
            <#elseif desglose_cab.ieps_total != "0.00" || desglose_cab.iva_total != "0.00" || total_traslados_ivacero gt 0 || tieneExento == true>
                ...
                <#if tiene_impuesto gt 0>
                    <cfdi:Impuestos TotalImpuestosTrasladados3="${total_impuestos_t?string["0.00"]}">
                </#if>
            </#if>
        </#if>
    <#--  </#if>  -->
        ...
        <#if desglose_cab.ieps_total != "0.00" || desglose_cab.iva_total != "0.00" || total_traslados_ivacero gt 0 || tieneExento == true>
            ...
            <#--  <#if desglose_cab.shipping_cost !="0">  -->
                    ...
                    <cfdi:Traslado Base="${desglose_cab.base_traslados}" Importe="${desglose_cab.totalTraslados}" Impuesto="002" TasaOCuota="${0.160000}" TipoFactor="Tasa" />
        ...
        <#if ImpLocalobj?has_content && ImpLocal  == true>
            <cfdi:Complemento>
                <#if transaction.custbody_efx_fe_total_impuesto_local?has_content>
        ...
        <cfdi:Complemento>
            ...
            <cce20:ComercioExterior Version="2.0"
                ...
                <#if json_direccion_field?has_content>
                    <cce20:Emisor>
                        <cce20:Domicilio Calle="${json_direccion.emisor.Calle}"
                    ...
                    <#if CEPropNumreg?has_content && CEpropResidenciaF?has_content>
                        <cce20:Propietario NumRegIdTrib="${CEPropNumreg}" ResidenciaFiscal="${CEpropResidenciaF}"/>
                    </#if>
                    <cce20:Receptor NumRegIdTrib="${CEreceptorNumR}">
                        <cce20:Domicilio Calle="${json_direccion.receptor.Calle}"
                    ...
                    <#if CEdestinatario?has_content>
                        <cce20:Destinatario NumRegIdTrib="${CEdestNumreg}" Nombre="${CEdestName}">
                            <cce20:Domicilio Calle="${json_direccion.destinatario.Calle}"
                ...
                <cce20:Mercancias>
                    <#list custom.items as customItem>
                        ...
                        <cce20:Mercancia
                            ...
                            ValorUnitarioAduana="${item.custcol_efx_fe_ce_val_uni_aduana?replace(',','')?number?string["0.000000"]}"
                            ValorDolares="${item.custcol_efx_fe_ce_val_dolares?replace(',','')?number?string["0.0000"]}">
                            <#if item.custcol_efx_fe_ce_des_especificas?has_content>
                                <cce20:DescripcionesEspecificas Marca="${item.custcol_efx_fe_ce_des_especificas}" NumeroSerie="${item.custcol_efx_fe_ce_des_numero_serie}" Modelo="${item.custcol_efx_fe_ce_des_modelo}"/>
                            </#if>
                        </cce20:Mercancia>
