...
<#if custom?has_content>
    ...
    <#if custom.dataFacTCambio?has_content>
      <#assign dataTCamFact = custom.dataFacTCambio>
    </#if>
...
<#if (record.currency=='Dólar EE.UU.' || record.currency == 'USA')>
    <#assign "moneda_sat" = 'USD DÓLAR EE.UU.'>
</#if>


<#if (record.custbody_efx_fe_moneda == 'Peso Mexicano' || record.currency=='Peso Mexicano' || record.currency=='MXN' || record.custbody_efx_fe_moneda == 'MXN')>
                    ...
                    <#if custom?has_content>
                        ...
                        <#else>
                            <td align="left" colspan="18" line-height="150%">
                                ...
                                <#if dataTCamFact?has_content>
                                  <#assign TCambioF = dataTCamFact?eval>
                                  <#list TCambioF as TCambio>
                                      <#if (linea.atributos.IdDocumento) == (TCambio.custbody_mx_cfdi_uuid)>
                                          <#assign equivaFac = 1 / (TCambio.exchangerate?number)>
                                          ${equivaFac?string["0.000000;;roundingMode=floor"]}
                                      </#if>
                                  </#list>
                                <#else>
                                  <#assign TCambioF = record.custbody_mx_plus_tc_facturas?eval>
                                  <#list TCambioF as TCambio>
                                      <#if (linea.atributos.IdDocumento) == (TCambio.custbody_mx_cfdi_uuid)>
                                          <#assign equivaFac = 1 / (TCambio.exchangerate?number)>
                                          ${equivaFac?string["0.000000;;roundingMode=floor"]}
