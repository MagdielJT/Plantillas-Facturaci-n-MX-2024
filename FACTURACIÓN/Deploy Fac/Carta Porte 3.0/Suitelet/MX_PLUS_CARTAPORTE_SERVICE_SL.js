 ...
 */
    (https, log, record, format, search, modcurrency, config, runtime, libCFDI, crypto, cartaporte_lib, xml, file) => {
        ...
        function convertToMexicoCityTime(date) {
            ...
            var mexicoCityOffset = -7 * 60; // Mexico City is UTC-5
        ...
        const onRequest = (scriptContext) => {
            try {
                ...
                if (fieldTransInternacional == true || fieldTransInternacional == 'T') {
                    ...
                    log.audit({ title: 'info_internacional 🐣🐣', details: info_internacional });
                    ...
                    Object.keys(obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"]).forEach((key) => {
                        if (key == 'RegimenAduanero' || key == 'EntradaSalidaMerc' || key == 'PaisOrigenDestino' || key == 'ViaEntradaSalida') {
                            delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"][key];
                        }
                    });
                    // obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"] = obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].filter((item) => item !== null);
                }
                ...
                obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Mercancias = cartaporte_lib.getMercancias(params.id, fieldTransInternacional, obj_json_cp.Emisor.Rfc, str_transInt);
                ...
                var datosPac = dataPac(obj_json_cp, id_transaccion, record_transaccion.getValue({ fieldId: 'tranid' }),record_transaccion);
        ...
        function dataPac(obj_json_cp, id_transaccion, tranid_text,recordTransaccion) {
            ...
            if (data_token.success) {
                ...
                var response_stamp = https.post({
                    // hardcodeado ⛳
                    url: 'https://services.sw.com.mx/v3/cfdi33/issue/json/v4',
                    headers: headers,
                ...
                // saveSendingMethod(recordTransaccion);
                    ...
                    if (response_json_stamp.data) {
                        ...
                        if (Object.keys(response_json_stamp.data.cfdi).length !== 0) {
                            ...
                            if (errorText.includes('Xml CFDI')) {
                                ...
                                return { msg: 'Error al timbrar carta porte: ' + response_json_stamp.messageDetail + '<b>Favor de llenar campos de Carta Porte en artículo</b>', success: false, fileID: 0 }
                            } else {
        ...
        const saveSendingMethod=(recordTrans)=>{
            try{
                log.debug({title:'entra a saveSending Method',details:'true'});
                let hasSendingMethod=recordTrans.getValue({fieldId:'custbody_psg_ei_sending_method'});
                if(!hasSendingMethod || hasSendingMethod==''){
                    ...
                    log.debug({title:'SALIENDO DE saveSending Method',details:'true'});
            
            }catch(err){
            log.error({title:'Error occurred in saveSendingMethod',details:err});
            }
        }
