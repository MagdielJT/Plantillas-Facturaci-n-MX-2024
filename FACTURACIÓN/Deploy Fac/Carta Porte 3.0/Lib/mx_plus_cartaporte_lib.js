    ...
    (log, record, search, libCFDI, runtime, config) => {
        function addMercanciaInternacionalInfo(record_transaccion,pais_cve) {
            ...
            try {
                ...
                let entraSalidaMer=record_transaccion.getText({
                    fieldId: "custbody_mxplus_entra_sali_mercan"
                });
                data_to_return.EntradaSalidMerc=entraSalidaMer;
        ...
        const getMercancias = (id_transaccion, es_Internacional, RFCImp, TraInte) => {
            var data_to_return = {
                PesoBrutoTotal: 0,
                UnidadPeso: '',
                NumTotalMercancias: '',
            ...
            let numPedi = record_transaccion.getValue({ fieldId: 'custbodyzh_pedimento_exportacion' });
            log.audit({ title: 'pedimento ☠️☠️☠️☠️☠️', details: [numPedi, TraInte] });
            partialMercanciaInfo.forEach((item, index) => {
                ...
                if (TraInte === 'Sí') {
                    item.DocumentacionAduanera = [
                        {
                            "TipoDocumento": "01",
                            "NumPedimento": numPedi,
                            "RFCImpo": RFCImp
                        }
                    ]
                    let entraSalidaMer = record_transaccion.getText({
                    ...
                    if (entraSalidaMer !== 'Entrada') {
                        delete item.DocumentacionAduanera[0].NumPedimento
                        item.DocumentacionAduanera = item.DocumentacionAduanera.filter(clean => clean !== null)
                    }
                }
                ...
                log.audit({ title: 'DocAduana ☠️☠️☠️☠️☠️', details: item.DocumentacionAduanera });
            });
        ...
        const fetchVehiculo = (id_vehiculo, pesoVehicular) => {
            var data_to_return = {
                ...
                "IdentificacionVehicular": {
                    "ConfigVehicular": '',
                    "PesoBrutoVehicular": pesoVehicular.toFixed(2) + '',
                    "PlacaVM": '',
                    "AnioModeloVM": ''
                }
