 ...
 */
    (record, search, runtime, log) => {
        ...
        const afterSubmit = (scriptContext) => {
            ...
            if (record_Tipo == 'invoice' || record_Tipo == 'cashsale'|| record_Tipo == 'creditmemo') {
                ...
                if (arr_items.length > 0) {
                    arr_items.forEach((item, index_arr) => {
                        ...
                        item.iva.base_importe=parseFloat(parseFloat(item.iva.base_importe).toFixed(2))
                        record_transaccion.setSublistValue({
                            sublistId: 'item',
                            fieldId: 'custcol_efx_fe_tax_json',
                            line: index_arr,
                ...
                if (parseFloat(sumatoria_importes.toFixed(2)) < obj_tax_transaccion.totalTraslados) {
                    ...
                    if (arr_items.length > 0) {
                        arr_items.forEach((item, index_arr) => {
                            ...
                            item.iva.base_importe=parseFloat(item.iva.base_importe.toFixed(2))
                ...
                // GET sumatoria de bases aunque no tenga que hacer ajustes de centavos
                if (arr_items.length > 0) {
                    sumatoria_bases=0;
                    arr_items.forEach((item, index_arr) => {
                ...
                if(obj_tax_transaccion.shipping_cost>0){
                    sumatoria_bases=parseFloat(sumatoria_bases.toFixed(2))+parseFloat(parseFloat(obj_tax_transaccion.shipping_cost).toFixed(2))
                }
                if (!tiene_ajuste) {
                ...
                obj_tax_transaccion.rates_iva_data.Iva_bases=parseFloat(obj_tax_transaccion.rates_iva_data.Iva_bases.toFixed(2)).toFixed(2);
