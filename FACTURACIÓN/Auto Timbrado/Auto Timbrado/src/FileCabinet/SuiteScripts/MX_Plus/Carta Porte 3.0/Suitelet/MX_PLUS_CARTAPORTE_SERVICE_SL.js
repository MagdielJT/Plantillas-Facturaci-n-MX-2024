 ...
 */
define(['N/https', 'N/log', 'N/record', 'N/format', 'N/search', 'N/currency', 'N/config', 'N/runtime', '../../Utilities/EFX_FE_Lib', 'N/crypto/random', '../Lib/mx_plus_cartaporte_lib', 'N/xml', 'N/file'],
    /**
 * @param{log} log
 */
    (https, log, record, format, search, modcurrency, config, runtime, libCFDI, crypto, cartaporte_lib, xml, file) => {
        ...
        function convertToMexicoCityTime(date) {
            ...
            var mexicoCityOffset = -6 * 60; // Mexico City is UTC-5
        // ZAHORI ZONA HORARIA TIMBRA COMO DE TIJUANA
            if(runtime.accountId.includes('5610235')){
                mexicoCityOffset = -7 * 60; //Tijuana
            }
        ...
        const fetchItemValues = (itemsId) => {
            ...
            try {
                ...
                const itemSearchColTipo = search.createColumn({ name: 'type' });
                const itemSearchColCdigoUPC = search.createColumn({ name: 'upccode' });
                const itemSearchColUnidadDeStockPrincipal = search.createColumn({ name: 'stockunit' });
                const itemSearchColSatProductCode = search.createColumn({ name: 'custitem_mx_txn_item_sat_item_code' });
                ...
                const itemSearch = search.create({
                    ...
                    columns: [
                        itemSearchColTipo,
                        itemSearchColCdigoUPC,
                        itemSearchColUnidadDeStockPrincipal,
                        itemSearchColSatProductCode
                    ],
                });
                ...
                for (let i = 0; i < itemSearchPagedData.pageRanges.length; i++) {
                    ...
                    itemSearchPage.data.forEach((result) => {
                        const tipo = result.getValue(itemSearchColTipo);
                        const cdigoUPC = result.getValue(itemSearchColCdigoUPC);
                        const unidadDeStockPrincipal = result.getValue(itemSearchColUnidadDeStockPrincipal);
                        const satProductCode = result.getValue(itemSearchColSatProductCode);
                        let typeR = result.recordType;
                        data_to_return.push({ typeR, cdigoUPC, unidadDeStockPrincipal, satProductCode })
                    });
                }
                ...
                log.error({ title: 'Error occurred in fetchItemValues', details: err });
            }
        ...
        const onRequest = (scriptContext) => {
            try {
                ...
                // const offset = -7; // Mexico City timezone offset from UTC in hours
                ...
                for (let i = 0; i < line_count; i++) {
                    var itemId = record_transaccion.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'item',
                        line: i,
                    });
                    item_arrays.push(itemId)
                }
                let item_data = fetchItemValues(item_arrays);
                log.audit({ title: 'item_data ✅✅✅', details: item_data });
                for (let i = 0; i < line_count; i++) {
                    ...
                    //     type: item_data[i].typeR,
                    //     id: articulo_id
                    // });

                    var claveProducto_id = item_data[i].satProductCode;
                    log.audit({
                    ...
                    var noIden = item_data[i].cdigoUPC
                    ...
                    obj_data_conceptos.Unidad = item_data[i].unidadDeStockPrincipal;
                ...
                if(obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[0].Domicilio.Localidad==''){
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[0].Domicilio.Localidad
                }
                ...
                if (JsonDirecciones_Parse.receptor.Rfc == 'XEXX010101000') {
                    obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].NumRegIdTrib = JsonDirecciones_Parse.receptor.NumRegIdTrib;
                    obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].ResidenciaFiscal = JsonDirecciones_Parse.receptor.ResidenciaFiscal;
                } else {
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].NumRegIdTrib;
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].ResidenciaFiscal;
                }
                ...
                var str_transInt = 'No'
                var fieldTransInternacional = record_transaccion.getValue({ fieldId: 'custbody_mx_plus_trans_internacional' });
                if (fieldTransInternacional == true || fieldTransInternacional == 'T') {
                    str_transInt = 'Sí';
                    var info_internacional = cartaporte_lib.addMercanciaInternacionalInfo(record_transaccion, JsonDirecciones_Parse.receptor.Pais);
                    obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].RegimenAduanero = info_internacional.RegimenAduanero;
                    obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].EntradaSalidaMerc = info_internacional.EntradaSalidMerc;
                    obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].PaisOrigenDestino = info_internacional.PaisOrigenDestino;
                    obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].ViaEntradaSalida = info_internacional.ViaEntradaSalida;

                } else {
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].RegimenAduanero
                }
                obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].TranspInternac = str_transInt;
                ...
                if(obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].Domicilio.Localidad ==''){
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].Domicilio.Localidad;
                }
                ...
                // Si es Internacional, no existe colonia, localidad y municipio
                if (fieldTransInternacional == true || fieldTransInternacional == 'T') {
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].Domicilio.Colonia;
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].Domicilio.Localidad;
                    delete obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Ubicaciones[1].Domicilio.Municipio
                }
                // ========================== MERCANCIAS ==========================
                // log.audit({ title: 'cartaporte_lib.getMercancias(params.id)', details: cartaporte_lib.getMercancias(params.id) });
                obj_json_cp.Complemento.Any[0]["cartaporte30:CartaPorte"].Mercancias = cartaporte_lib.getMercancias(params.id, fieldTransInternacional);
                // ========================== FIGURA TRANSPORTE ==========================
        ...
        const fetchConnectionInfo=()=>{
            let data_to_return={
                testMode:false,
                url:''
            }
            try{
                var datosConexionId = runtime.getCurrentScript().getParameter({name: 'custscript_efx_fe_connect_pac_data'});
                let testingMode=false;
                const customrecordEfxFeMtdEnvioSearchFilters= [
                    ['internalid', search.Operator.ANYOF, datosConexionId],
                    'AND',
                    ['isinactive', search.Operator.IS, 'F'],
                ];
                
                const customrecordEfxFeMtdEnvioSearchColPruebas = search.createColumn({ name: 'custrecord_efx_fe_mtd_env_test' });
                
                const customrecordEfxFeMtdEnvioSearch = search.create({
                    type: 'customrecord_efx_fe_mtd_envio',
                    filters: customrecordEfxFeMtdEnvioSearchFilters,
                    columns: [
                        customrecordEfxFeMtdEnvioSearchColPruebas,
                    ],
                });

                
                const customrecordEfxFeMtdEnvioSearchPagedData = customrecordEfxFeMtdEnvioSearch.runPaged({ pageSize: 1 });
                for (let i = 0; i < customrecordEfxFeMtdEnvioSearchPagedData.pageRanges.length; i++) {
                    const customrecordEfxFeMtdEnvioSearchPage = customrecordEfxFeMtdEnvioSearchPagedData.fetch({ index: i });
                    customrecordEfxFeMtdEnvioSearchPage.data.forEach((result) => {
                        const pruebas = result.getValue(customrecordEfxFeMtdEnvioSearchColPruebas);
                        testingMode=pruebas;
                        
                    });
                }
                if(testingMode==true || testingMode=='T'){
                    data_to_return.testMode=true;
                    data_to_return.url='https://services.test.sw.com.mx/v3/cfdi33/issue/json/v4'
                }else{
                    data_to_return.testMode=false;
                    data_to_return.url='https://services.sw.com.mx/v3/cfdi33/issue/json/v4'
                }
                
                
            }catch(err){
            log.error({title:'Error occurred in fetchConnectionInfo',details:err});
            }
            return data_to_return;
        }
        function dataPac(obj_json_cp, id_transaccion, tranid_text) {
            ...
            if (data_token.success) {
                ...
                let connectionInformation=fetchConnectionInfo();
                log.debug({title:'connectionInformation',details:connectionInformation});
                ...
                var idFolder = cartaporte_lib.getFolderTimb()
                if (response_stamp.code == 200) {
                    ...
                    if (response_json_stamp.data) {
                        ...
                        if (Object.keys(response_json_stamp.data.cfdi).length !== 0) {
                            var file_id_saved = save_xml_file(response_json_stamp.data.cfdi, id_transaccion, tranid_text, idFolder);
                            var file_id_generated = save_json_file(JSON.stringify(obj_json_cp), id_transaccion, tranid_text, 'txt', idFolder);
                            ...
                            var errorText = response_json_stamp.messageDetail;

                            if (errorText.includes('Xml CFDI')) {
                                log.debug({ title: 'cayó en Xml CFDI', details: 'true' });

                                return { msg: 'Error al timbrar carta porte: ' + response_json_stamp.messageDetail + 'Favor de llenar campos de Carta Porte en artículo', success: false, fileID: 0 }
                            } else {
                    ...
                    save_json_file(JSON.stringify(obj_json_cp), id_transaccion, tranid_text, 'txt', idFolder)
                    if (obj_response_stamp.messageDetail == null) {
                        var errorText = obj_response_stamp.message;

                        if (errorText.includes('Xml CFDI')) {
                            log.debug({ title: 'cayó en Xml CFDI', details: 'true' });

                            return { msg: 'Error al timbrar carta porte: ' + obj_response_stamp.message + 'Favor de llenar campos de Carta Porte en artículo', success: false, fileID: 0 }
                        } else {
        ...
        function save_json_file(data, id_transaccion, tranid_text, extension, idFolder) {
            try {
        ...
        function save_xml_file(data, id_transaccion, tranid_text, idFolder) {
            try {
