    ...
    (https, log, record, search, runtime, file, task, functions, constants, access_pac, config, format, invoice_constants, moment) => {
        ...
        const sacaCalculosTotales = (recordTrans, cantidadLineas) => {
            try {
                ...
                data_to_return.total_transaccion = (parseFloat(data_to_return.subtotal) + parseFloat(data_to_return.TotalImpuestosTrasladados) - parseFloat(data_to_return.descuentos) - parseFloat(data_to_return.TotalImpuestosRetenidos)) || 0
                data_to_return.total_transaccion = parseFloat(data_to_return.total_transaccion).toFixed(2);
                data_to_return.TotalImpuestosTrasladados = parseFloat(data_to_return.TotalImpuestosTrasladados) || 0;
        ...
        const fetchConceptos = (record_trans, id_transaccion, typeOfRecord) => {
            try {
                ...
                log.debug({ title: 'items_custom_object 🐝', details: items_custom_object });
                ...
                for (var i = 0; i < number_items; i++) {
                    ...
                    if (item_tax_json != '') {
                        ...
                        log.debug({ title: 'unidad', details: unidad });
                        // log.debug({title:'items_custom_object[other_index].satUnitCode',details:items_custom_object[other_index].satUnitCode});
                        if ((unidad == '' || unidad == null || unidad == undefined) && claveUnidad == 'H87') {
                            unidad = 'Pza';
                        }
                        ...
                        if (!taxDetai.toLowerCase().includes('exento')) {
                            hasExento = false;
                        } else {
                            hasExento = true;
                        }
                ...
                if (hasExento == true && recalculateConceptos.totalTraslados === '0.00') {
        ...
        const execute_RecalculateConceptos = (arrayConceptos, arrImpuestos, discountamount, totalNetsuiteTransaccion) => {
            try {
                ...
                log.audit({title:'arrTrasladosTotalesExen 🛹',details:arrTrasladosTotalesExen});
                ...
                log.audit({title:'objTrasladosTotales 🛹',details:objTrasladosTotales});
