 ...
 * @NApiVersion 2.1
 */
define(['N/config', 'N/query', 'N/runtime', 'N/search','N/record','N/file'], (config, query, runtime, search,record,file) => {
  ...
  const createFile = (prefix,extension,tranid,content,folder) => {
    try {
      var fileObj = file.create({
        name: prefix + tranid + extension,
      ...
      log.error({ title: 'Error occurred in createFile', details: err });
    }
  }
  const getFolderTimb = () => {
    try {
