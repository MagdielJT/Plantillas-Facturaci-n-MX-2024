 ...
 */
define(['../server/mx_plus_cancelacion_cfdi', '../server/_mxplus_payments', '../server/_mxplus_invoice', '../lib/functions_gbl', '../server/_mxplus_itemfulfillment', '../Adendas/lib_adendas/adendas_functions'],
  (cancel_cfdi, stampPayment, invoice, functionsGBL, itemfulfillment, adendas) => {
    const onRequest = scriptContext => {
      try {
        const { request, response } = scriptContext
        let { actionMode, recordId, recordType, motCancel, uuidRela, reprocessedGBL, idFolder } = request.parameters
        let result = { success: false, data: [], endpoint: '', details: '' }
        // Most likely to come from Autotimbrado
        if (!actionMode) {
          ...
          if (request.body) {
            ...
            reprocessedGBL = body.reprocessedGBL;
            idFolder = body.idFolder;
            uuidRela = body.uuidRela;
            motCancel = body.motCancel;
          }
        }
        if (actionMode) {
            ...
            case 'stamp_payment':
              ...
              if (JSON_STRUCTURE) {
                ...
                if (isSend.status == 'error') {
                  ...
                  if (result.data) {
                    result.success = true;
                    result.msg = 'Pago timbrado con éxito';
                ...
                log.debug({title:'result PAGO',details:result});
              }
              break
            case 'create_adenda':
