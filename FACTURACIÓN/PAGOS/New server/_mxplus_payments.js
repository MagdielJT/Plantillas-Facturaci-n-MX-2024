    ...
    try {
      ...
      if (Object.keys(basePmt).length) {
        ...
        dataGenerate[BASE.Receptor][RECEPTOR.Rfc] = receptor[CUSTOMER.FIELDS.RFC]
          ...
          let otherCustomerInformation = getCustomerBasicInformationPMT(receFactoraje[FACTORAJE.CHECK_FAC].value != true ? basePmt[PAYMENT.FIELDS.CUSTOMER].value : receFactoraje[FACTORAJE.CLIENT].value)
          ...
          dataGenerate[BASE.Receptor][RECEPTOR.DomicilioFiscalReceptor] = receptor[CUSTOMER.FIELDS.RFC] == 'XAXX010101000' || receptor[CUSTOMER.FIELDS.RFC] == 'XEXX010101000' ? dataGenerate[BASE.LugarExpedicion] : otherCustomerInformation.DomicilioFiscalReceptor
        ...
        paymentDetails.forEach(invoice => {
          ...
          lineInv[DOC_REL.NumParcialidad] = Number(partiality?.parcialidad ?? 1).toFixed(0)
            log.debug({title:'invoice[INVOICE.FIELDS.TOTAL].value',details:invoice[INVOICE.FIELDS.TOTAL].value});
            log.debug({title:'invoice[INVOICE.FIELDS.EXCHANGE_RATE].value',details:invoice[INVOICE.FIELDS.EXCHANGE_RATE].value});
            lineInv[DOC_REL.ImpSaldoAnt] = Number(invoice[INVOICE.FIELDS.TOTAL].value).toFixed(2)
            lineInv[DOC_REL.ImpPagado] = Number(invoice[INVOICE.FIELDS.AMOUNT_PAID].value ).toFixed(2)
            log.debug({title:'lineInv[DOC_REL.ImpPagado] BF',details:lineInv[DOC_REL.ImpPagado]});
          lineInv[DOC_REL.ImpSaldoInsoluto] = Number(invoice[INVOICE.FIELDS.AMOUNT_REMAINING].value).toFixed(2)
            ...
            log.debug({title:'lineInv[DOC_REL.ImpPagado]',details:lineInv[DOC_REL.ImpPagado]});
          ...
          if (Object.keys(tax_json).length) {
            if (Object.keys(tax_json.rates_iva).length) {
              Object.entries(tax_json.bases_iva).forEach(([key, value]) => {
                const trasladoDR = {}
                let tax = key / 100
                  log.debug({title:'invoice[INVOICE.FIELDS.TOTAL].value AGAIN',details:invoice[INVOICE.FIELDS.TOTAL].value});
                  let auxBaseDR = receFactoraje[FACTORAJE.CHECK_FAC].value != true ? invoice[INVOICE.FIELDS.TOTAL].value / (1 + tax) : (invoice[INVOICE.FIELDS.TOTAL].value - invoice[INVOICE.FIELDS.FACTORAJE].value) / (1 + tax)
                ...
                trasladoDR[DOC_REL.TasaOCuotaDR] = Number(key / 100).toFixed(6)
                  log.debug({ title: 'auxBaseDR', details: auxBaseDR });
                  ...
                  log.debug({ title: 'auxImp', details: auxImp });
                ...
                // trasladoDR[DOC_REL.ImporteDR] = Number(tax_json.rates_iva_data[key]).toFixed(6)
                  ...
                  log.debug({title:'auxBaseP SUM 🐣',details:auxBaseP});
                  ...
                  log.debug({ title: 'Concptos P', details: { typeof: auxBaseP, typeof: auxImpP } });
