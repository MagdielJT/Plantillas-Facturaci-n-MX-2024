 ...
 */ (file, https, log, record, query, search, constants, access_pac, functions, moment) => {
  ...
  const getEmisorRFC = idSubsidiary => {
    try {
      ...
      log.error({ title: 'Error occurred in getEmisorRFCpmt', details: err })
    }
  }
  const generateJSON = (recordId, recordType) => {
    ...
    try {
      ...
      if (Object.keys(basePmt).length) {
        ...
        dataGenerate[BASE.Receptor][RECEPTOR.Nombre] = receptor[CUSTOMER.FIELDS.RFC] == 'XAXX010101000' ? 'PUBLICO GENERAL' : otherCustomerInformation.NombreSAT
        ...
        let totalTI = ''
        let auxImpT = 0
        paymentDetails.forEach((invoice, index) => {
          const tax_json = invoice[INVOICE.FIELDS.TAX_JSON].value
          auxImpT = receFactoraje[FACTORAJE.CHECK_FAC].value == true ? auxImpT + Number(dataApply[index].amount) : auxImpT + Number(dataApply[index].amount)
          ...
          if (Object.keys(tax_json).length) {
            ...
            } else if (Object.keys(tax_json.rates_retencion).length) {
              Object.keys(tax_json.rates_retencion).forEach(key => {
                switch (key) {
                  case 'IVA':
                    break
                  case 'IEPS':
        ...
        paymentDetails.forEach(invoice => {
          ...
          if (Object.keys(tax_json).length) {
            if (Object.keys(tax_json.rates_iva).length) {
              ...
              // pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS.MontoTotalPagos] += pago[PAGO.MonedaP] !== 'MXN' ? Number(invoice[INVOICE.FIELDS.AMOUNT_PAID].value) * parseFloat(pago[PAGO.TipoCambioP]) : Number(basePmt[PAYMENT.FIELDS.TOTAL].value)
              let MTP = pago[PAGO.MonedaP] !== 'MXN' ? Number(invoice[INVOICE.FIELDS.AMOUNT_PAID].value) * parseFloat(pago[PAGO.TipoCambioP]) : Number(basePmt[PAYMENT.FIELDS.TOTAL].value)
              pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS.MontoTotalPagos] = MTP
              // })
            } else {
        ...
        paymentDetails.forEach((invoice, index) => {
          ...
          lineInv[DOC_REL.ImpPagado] = Number(dataApply[index].amount).toFixed(2)
          // lineInv[DOC_REL.ImpPagado] = Number(invoice[INVOICE.FIELDS.AMOUNT_PAID].value).toFixed(2)
          lineInv[DOC_REL.ImpSaldoInsoluto] = Number(invoice[INVOICE.FIELDS.AMOUNT_REMAINING].value).toFixed(2)
          if (parseFloat(pago[PAGO.Monto]) < parseFloat(lineInv[DOC_REL.ImpPagado])) {
            log.debug({title:'MONTO MENOR AL IMP',details:'MONTO MENOR AL IMP'});
            lineInv[DOC_REL.ImpPagado] = pago[PAGO.Monto]
            lineInv[DOC_REL.ImpSaldoInsoluto] = (Number(lineInv[DOC_REL.ImpSaldoAnt]) - Number(lineInv[DOC_REL.ImpPagado])).toFixed(2)
            pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS.MontoTotalPagos] = parseFloat(pago[PAGO.Monto]) * parseFloat(pago[PAGO.TipoCambioP])
            pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS.MontoTotalPagos] = parseFloat(pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS.MontoTotalPagos]).toFixed(2)
          }
          ...
          if (Object.keys(tax_json).length) {
            if (Object.keys(tax_json.rates_iva).length) {
              Object.keys(tax_json.bases_iva).forEach(key => {
                const trasladoDR = {}
                let tax = key / 100
                let auxBaseDR = receFactoraje[FACTORAJE.CHECK_FAC].value != true ? dataApply[index].amount / (1 + tax) : (dataApply[index].amount - invoice[INVOICE.FIELDS.FACTORAJE].value) / (1 + tax)
                // let auxBaseDR = receFactoraje[FACTORAJE.CHECK_FAC].value != true ? value : (invoice[INVOICE.FIELDS.TOTAL].value - invoice[INVOICE.FIELDS.FACTORAJE].value) / (1 + tax)
                ...
                auxBaseP = auxBaseP + parseFloat(trasladoDR[DOC_REL.BaseDR])
                // auxBaseP = receFactoraje[FACTORAJE.CHECK_FAC].value == true ? auxBaseP + parseFloat(trasladoDR[DOC_REL.BaseDR]) : 0
                // auxBaseP = receFactoraje[FACTORAJE.CHECK_FAC].value == true ? auxBaseP + auxBaseDR : 0
                auxImpP = auxImpP + parseFloat(trasladoDR[DOC_REL.ImporteDR])
                ...
                if (Object.keys(tax_json.rates_ieps).length) {
                  Object.entries(tax_json.bases_ieps).forEach(([keyy, value]) => {
                    const trasIeps = {}
                    let taxx = keyy / 100
                    let baseIeps = dataApply[index].amount / ((1+taxx)*(1+tax))
                    trasIeps[DOC_REL.BaseDR] = Number(baseIeps).toFixed(6)
                    trasIeps[DOC_REL.ImpuestoDR] = '003'
                    trasIeps[DOC_REL.TipoFactorDR] = 'Tasa'
                    trasIeps[DOC_REL.TasaOCuotaDR] = Number(taxx).toFixed(6)
                    trasIeps[DOC_REL.ImporteDR] = Number(baseIeps * taxx).toFixed(6)
                    basePieps = basePieps + Number(trasIeps[DOC_REL.BaseDR])
                    impPieps = impPieps + Number(trasIeps[DOC_REL.ImporteDR])
                    lineInv[DOC_REL.ImpuestosDR][DOC_REL.TrasladosDR].push(trasIeps)
                    log.debug({title:'INDEX TRASLADOS',details:index});
                  })
                }
              })
            } else {
          ...
          if (Object.keys(tax_json).length) {
            if (Object.keys(tax_json.rates_iva).length) {
              Object.entries(tax_json.bases_iva).forEach(([key, value]) => {
                ...
                // impuestoAux[key][IMPUESTOS.BaseP] += Number(baseP) / pago[PAGO.TipoCambioP]
                ...
                // impuestoAux[key][IMPUESTOS.ImporteP] += Number(impP) * pago[PAGO.TipoCambioP]
                pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS[totalTB]] = parseFloat(impuestoAux[key][IMPUESTOS.BaseP]) * parseFloat(pago[PAGO.TipoCambioP])
                pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS[totalTB]] = Number(pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS[totalTB]]).toFixed(2)
                pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS[totalTI]] = Number(impuestoAux[key][IMPUESTOS.ImporteP]) * parseFloat(pago[PAGO.TipoCambioP])
                pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS[totalTI]] = Number(pagos[COMPLEMENTO.Pago20][PAGOS.Totales][PAGOS[totalTI]]).toFixed(2)
              })
              if (Object.keys(tax_json.rates_ieps).length) {
                Object.keys(tax_json.bases_ieps).forEach(key => {
                  if (!taxPieps?.[key]) {
                    taxPieps[key] = {}
                    Object.keys(IMPUESTOS).forEach(entry => (taxPieps[key][entry] = 0))
                  }
                  taxPieps[key][IMPUESTOS.BaseP] = Number(basePieps).toFixed(6)
                  taxPieps[key][IMPUESTOS.ImpuestoP] = '003'
                  taxPieps[key][IMPUESTOS.TipoFactorP] = 'Tasa'
                  taxPieps[key][IMPUESTOS.TasaOCuotaP] = Number(key / 100)
                  taxPieps[key][IMPUESTOS.ImporteP] = Number(impPieps).toFixed(6)
                })
              }
            } else {
        ...
        if (Object.keys(taxPieps).length) {
        ...
        log.debug({title:'New IMPUESTOS',details:impuestoAux});
        log.debug({title:'New IMPUESTOS P',details:taxPieps});
        // if (Object.keys(tax_json.rates_ieps).length) {
        //   Object.values(taxPieps).forEach(taxIeps => pago[PAGO.ImpuestosP][PAGO.TrasladosP].push(taxIeps))
        // }
        arrPagos.push(pago)
